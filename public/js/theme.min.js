(function ($) {
    $(window).unload(function () {
        if (theme_object.post_id) {
            $.ajax({
                url: theme_object.ajax_url,
                type: "get",
                data: {action: "post_views_log", post_id: theme_object.post_id},
                async: false
            });
        }
    });
...
jQuery(function ($) {
    $("body").on("change", '.post-like-button [type="checkbox"]', function () {
        var $this = $(this);
        var $postLikeButton = $this.closest(".post-like-button");
        var $postLikesWrapper = $(".social-act");
        $.ajax({
            url: theme_object.ajax_url,
            type: "post",
            data: {
            ...
            beforeSend: function () {
                $postLikeButton.attr("disabled", "disabled");
            },
            error: function () {
                $postLikeButton.removeAttr("disabled");
                $this.prop("checked", !$this.prop("checked"));
            },
            success: function (response) {
                if ($postLikeButton.find(".post-like-count").size()) {
                    $postLikeButton.find(".post-like-count").text(response.likes || 0);
                }
                if ($postLikeButton.find(".post-dislike-count").size()) {
                    $postLikeButton.find(".post-dislike-count").text(response.dislikes || 0);
                }
                if (response.likable) {
                    $postLikesWrapper.find(".post-like-button").removeAttr("disabled").removeClass("disabled");
                    $postLikesWrapper.find('.post-like-button [type="checkbox"]').removeAttr("disabled").removeClass("disabled");
                } else {
                    $postLikesWrapper.find('.post-like-button [type="checkbox"]').attr("disabled", "disabled");
                    $postLikeButton.attr("disabled", "disabled").addClass("disabled");
                }
                if (response.restrict) {
                    $postLikeButton.attr("disabled", "disabled");
                }
                $postLikesWrapper.find('.post-like-button [type="checkbox"]').not($this).prop("checked", false);
                if (!response.login_required) {
                    if ($this.prop("checked") == true) {
                        $postLikeButton.find("i").addClass("clicked");
                    } else {
                        $postLikeButton.find("i").removeClass("clicked");
                    }
                }
                if (response.login_required) {
    ...
    $(".post-like-message").click(function () {
        $(this).empty();
    });
});
jQuery(function ($) {
    $("body").on("change", '.comment-like-button [type="checkbox"]', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.restrict) {
                    $comment.find(".comment-like-button").attr("disabled", "disabled");
                }
                $comment.find('.comment-like-button [type="checkbox"]').not($this).prop("checked", false);
                if (!response.login_required) {
                    if ($this.prop("checked") == true) {
                        $commentLikeButton.find("i").addClass("clicked");
                    } else {
                        $commentLikeButton.find("i").removeClass("clicked");
                    }
                }
                if (response.login_required) {
                    $this.prop("checked", false);
...
jQuery(function ($) {
    var starts = $(".stars-wrapper svg"), goldenWidth = percentageWidth($(".stars-wrapper .golden-rate")), goldenColor = $(".stars-wrapper .golden-rate").css("background-color"), startsContainer = $(".post-star-rating"), startsWrapper = $(".post-star-rating .stars-wrapper");
    starts.hover(function () {
        var index = $(this).index();
        $(".stars-wrapper .golden-rate").css({width: index * 20 + "%", backgroundColor: "#ff637d"});
    }, function () {
        $(".stars-wrapper .golden-rate").css({width: goldenWidth + "%", backgroundColor: goldenColor});
    });
    starts.on("click", function () {
        ...
        $.ajax({
            url: theme_object.ajax_url,
            type: "post",
            data: {action: "post_star_rating", post_id: theme_object.post_id, vote: index,},
            beforeSend: function () {
...
jQuery(function ($) {
    $(".playlist .playlist-nav li").on("click", function () {
        ...
        var media = $mediaItems.eq($(this).index()).find(".video-js")[0];
        media.player.play();
    });
});
    ...
    $(".followings .unfollow").on("click", function (e) {
        ...
        dialog(theme_followed_object.unfollow_confirmation, function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.type == "unfollow") {
                        $this.closest(".followed-item").remove();
                    }
                }
            });
        }, function () {
...
function formatCurrency(num, isRial, symbol) {
    ...
    num = Math.floor(num / (isRial ? 1000 : 100)).toString();
...
function customAlert(message, className, okCallback) {
    if (className.length > 0) {
        jQuery("#alert-dialog .modal-body p").addClass(className);
    } else {
        jQuery("#alert-dialog .modal-body p").removeAttr("class");
    }
    ...
    jQuery.ajax({
        ...
        success: function (response) {
            if (response.code == "200" || response.code == "300") {
                ...
                form.find("#response-msg").text(response.msg).css("display", "block").css("visibility", "visible");
            }
        ...
        jQuery.ajax({
            ...
            success: function (response) {
                if (response.code == "200") {
                    ...
                    jQuery("#response-msg").addClass("text-danger");
                }
...
jQuery(function ($) {
    ...
    if ($("body").hasClass("logged-in")) {
        $.ajax({
            ...
            type: "post",
            dataType: "json",
            data: {action: "check_user_wishlist", post_id: theme_object.post_id},
            success: function (response) {
                if (response.code == "200") {
    ...
    $(".wishlist-btn").click(function () {
        if (!$("body").hasClass("logged-in")) {
            ...
            $.ajax({
                url: theme_object.ajax_url,
                type: "post",
                data: {action: "get_categories", post_id: $(this).data("id")},
                success: function (response) {
                    $("#category-selector select").html(response);
                    showNewCategory(document.getElementById("select-category"));
                }
            });
        }
    });
    $(".bookmark-btn").click(function () {
        if (!$("body").hasClass("logged-in")) {
            ...
            $.ajax({
                ...
                type: "post",
                dataType: "json",
                data: {action: "add_to_bookmark", post_id: elem.data("id")},
                success: function (response) {
                    if (response.code == "200") {
    ...
    $("#remove-from-wishlist").click(function () {
        ...
        dialog(profile_object.remove_from_wishlist_confirmation, function () {
            $.ajax({
                ...
                type: "post",
                dataType: "json",
                data: {action: "remove_from_wishlist", post_id: theme_object.post_id},
                success: function (response) {
                    if (response == "200") {
...
function submitEditCategory(elem) {
    ...
    jQuery.ajax({
        ...
        success: function (response) {
            ...
            if (response.code == "200") {
                jQuery(".editing").find(".category-title a").text(form.find("#category-title-input").val());
            } else {
                customAlert(response.msg, "text-warning", function () {
                });
            }
...
jQuery(function ($) {
    $(".remove-category-btn").on("click", function (e) {
        ...
        dialog(profile_object.delete_folder_confirmation, function () {
            ...
            $.ajax({
                ...
                type: "post",
                dataType: "json",
                data: {action: "remove_category", category_id: cat_id},
                success: function (response) {
    ...
    $(".remove-from-wishlist").on("click", function (e) {
        ...
        var row = elem.closest(".bookmark-item");
        dialog(profile_object.remove_from_wishlist_confirmation, function () {
            var innerHtml = elem.html();
            elem.addClass("disabled");
            $.ajax({
    ...
    $(".change-wishlist-status").on("click", function (e) {
        ...
        $.ajax({
            ...
            type: "post",
            dataType: "json",
            data: {action: "change_wishlist_status", id: id},
            success: function (response) {
    ...
    if (!jQuery("body").hasClass("logged-in")) {
        ...
        if (jQuery("form.quiz-form").serializeArray().length > 0) {
            ...
            jQuery.ajax({
                ...
                type: "post",
                dataType: "json",
                data: {action: "submit_quiz", answers: form.serializeArray()},
                success: function (response) {
...
function getQuiz(elem) {
    if (!jQuery("body").hasClass("logged-in")) {
        ...
        jQuery.ajax({
            ...
            type: "post",
            dataType: "json",
            data: {action: "get_quiz", id: elem.attr("data-id")},
            success: function (response) {
                if (response.code == 200) {
...
jQuery(function ($) {
    if (typeof news_ticker_object != "undefined" && news_ticker_object.interval) {
        setInterval(function () {
            refreshNewsTicker(news_ticker_object);
        }, news_ticker_object.interval * 1000);
    }
});
function refreshNewsTicker(options) {
    var $news_ticker = jQuery("#news-ticker");
    jQuery.ajax({
        url: options.ajax_url, type: "post", data: {action: "update_news_ticker"}, beforeSend: function () {
            $news_ticker.find(".news-items").css("opacity", 0.5);
        }, success: function (response) {
...
jQuery(function ($) {
    $(".newsletter-form").submit(function (e) {
        ...
        jQuery.ajax({
            url: theme_object.ajax_url,
            type: "post",
            data: {action: "newsletter_subscribe", email: $this.find("[name=email]").val(),},
            beforeSend: function () {
